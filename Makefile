...
./target/release/s2tw: $(shell find . -type f -iname '*.rs' -o -name 'Cargo.toml' | sed 's/ /\\ /g')
	...
	cd $$PWD
	OPENCC_LIB_DIRS="$$OPENCC_PATH/linux/lib" OPENCC_INCLUDE_DIRS="$$OPENCC_PATH/linux/include/opencc" OPENCC_STATIC=1 OPENCC_LIBS=opencc OPENCC_DYLIB_STDCPP=1 cargo build --release
